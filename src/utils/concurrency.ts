 ...
 * Concurrency Manager
 *
 * Manages concurrency limits for background tasks with three-level granularity:
 * 1. Default limit (fallback)
 * 2. Provider-specific limit
 * 3. Model-specific limit (highest priority)
 */
 ...
 * Concurrency Manager
 *
 * Implements three-level granularity concurrency control:
 * - Model-specific limit (highest priority)
...
export class ConcurrencyManager {
   ...
   *
   * Hands off to next waiter if any, otherwise decrements count
   */
   ...
   * Clear all state
   *
   * Used during manager cleanup/shutdown. Cancels all pending waiters.
   */
  clear(): void {
